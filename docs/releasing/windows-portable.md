...
构建和打包阶段只生成未签名 ZIP 与 manifest。离线签名流程另行执行：使用受控环境中的 ECDSA P-256 私钥签署 canonical manifest，随后将 `manifest.json`、`manifest.sig` 和 ZIP 作为 GitHub Releases 资产发布。私钥不进入仓库、CI、ZIP 或客户端；客户端只内置经批准的 SPKI DER 公钥。后续如增加自有域名或 CDN，只替换静态入口，不改变签名信任根。
