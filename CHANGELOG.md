...
- Desktop 已接入 DPAPI 账号流程、头像本地缓存和多账号 UI；Microsoft Public Client 已完成注册准备并收到 Mojang AppID allow-list 审核通过通知，真实登录仍待 Windows 10/11 实机验收。
- 修复官方版本清单因历史版本 ID 含空格而整体显示 `VERSION_METADATA_INVALID` 的问题；版本目录安全校验现在允许合法的内部空格，同时仍拒绝路径分隔符、空白边界和点目录。
- 首次使用流程新增游戏根目录后，版本页和下载页会重新读取持久化设置与根目录列表，不需要重启启动器即可继续配置。
